...
public class Main {
	...
	public static void storePartyAnswers(String path, String[] submitInfo, Question[] answers) {
		...
		float y = Float.parseFloat(submitInfo[9]);
		...
		for(int num = 0; num < 9; num++) {
			if(answers[num] instanceof MultiChoiceQuestion) {
				...
				float x  = Float.parseFloat(submitInfo[num]);
				float z = (float) answers[num].getAns();
				finalSubmit[num] = String.valueOf(Math.round(((x * y + z) / (y + 1)) * 1000) / 1000f);
			}
		}
	...
	//	AND NOT VITAL TO THE UNDERLYING OPERATIONS OF THE APPLICATION
